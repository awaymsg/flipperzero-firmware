...
#include "../subghz_i.h"
#include "../helpers/subghz_debruijn.h"
...
void subghz_opensesame_draw(Canvas* canvas, SubGhzOpenSesameModel* model) {
    ...
    case SubGhzOpenSesameModelStatusTx:
        elements_text_box(
            ...
            AlignCenter,
            AlignCenter,
            string_get_cstr(model->debruijn_str),
            true);
...
void subghz_opensesame_enter(void* context) {
    ...
    with_view_model(instance->view, (SubGhzOpenSesameModel* model) {
        DeBruijn(model->codelength, model->debruijn_str);
        return true;
    });
}
...
SubGhzOpenSesame* subghz_opensesame_alloc() {
    ...
    with_view_model(
        subghz_opensesame->view, (SubGhzOpenSesameModel* model) {
            ...
            string_init(model->debruijn_str);
            return true;
        });
...
void subghz_opensesame_free(SubGhzOpenSesame* subghz_opensesame) {
    ...
    with_view_model(
        subghz_opensesame->view, (SubGhzOpenSesameModel* model) {
            ...
            string_clear(model->debruijn_str);
            return true;
        });
