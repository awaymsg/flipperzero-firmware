...
bool subghz_scene_start_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeBack) {
        ...
        if(event.event == SubmenuIndexReadRAW) {
            ...
            return true;
        } else if(event.event == SubmenuIndexOpenSesame) {
            scene_manager_set_scene_state(
                subghz->scene_manager, SubGhzSceneStart, SubmenuIndexOpenSesame);
            scene_manager_next_scene(subghz->scene_manager, SubGhzSceneOpenSesame);
            return true;
